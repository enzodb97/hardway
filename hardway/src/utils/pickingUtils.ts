...
// Exportar un pedido individual a PDF
/**
 * Exporta la información de un pedido a PDF, incluyendo número de pedido y productos (id, nombre, cantidad)
 * @param pedido Objeto con al menos { id, productos: [{ id, nombre, cantidad }] }
 */

// Definir el tipo de producto para el picking
export type PickingProducto = {
  id: number | string;
  nombre: string;
  cantidad: number;
  rack: string;
...
export const exportarPedidoPDF = (pedido: {
  ...
  const title = `Pedido N° ${pedido.id}`;
